<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Formulario para poder actualizar un nivel academico</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.6/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-4Q6Gf2aSP4eDXB8Miphtr37CMZZQ5oXLH2yaXMJ2w8e2ZtHTl7GptT4jmndRuHDT" crossorigin="anonymous">
</head>
<body>
    <?php
    $codigo = $_POST['hidden_codigoa'];
    // Buscar el codigo en la base de datos en la tabla de niveles academicos
    require_once('conexion.php');
    $sql = "SELECT * FROM nivelesacademicos WHERE cod_nivel_acad = '$codigo'";
    // Ejecutar la consulta en la base de datos
    $ejecutar = mysqli_query($conexion, $sql);
    // Convertir los datos a un array en PHP
    $datos = mysqli_fetch_assoc($ejecutar);
    ?>
    <div class="container">
        <h1>Modificar Nivel Academico</h1>
        <!-- crud_academicos.php es el archivo que realizara los procesos 
        crud en la bd -->
        <form action="crud_academicos.php" method="post">
            <label for="txt_codigo" class="form-label">Código</label>
            <input type="number" name="txt_codigo" id="txt_codigo" 
                value="<?php echo $codigo;?>" class="form-control" readonly>
            <label class="form-label" for="txt_nombre">Nombre</label>
            <input type="text" name="txt_nombre" id="txt_nombre" 
                value="<?php echo $datos['nombre'];?>" class="form-control">
            <label for="txt_descripcion" class="form-label">Descripción</label>
            <input type="text" name="txt_descripcion" id="txt_descripcion" 
                value="<?= $datos['descripcion']?>" class="form-control">
            <button type="submit" name="actualizar" class="form-control btn btn-primary">
                Actualizar Nivel Academico
            </button>
        </form>

</body>
</html>